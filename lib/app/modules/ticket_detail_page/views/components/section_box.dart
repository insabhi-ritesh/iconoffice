...
import 'package:insabhi_icon_office/app/modules/ticket_detail_page/views/components/send_message.dart'
...
Widget sectionBox(String title, Widget content) {
  return Container(
    ...
    // ✅ ClipRRect so the rounded corners still clip the expanded content
    child: ClipRRect(
      borderRadius: BorderRadius.circular(12),
      child: Theme(
        // ✅ Remove the default ExpansionTile dividers that cause layout issues
        data: ThemeData().copyWith(dividerColor: Colors.transparent),
        child: ExpansionTile(
          tilePadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
          childrenPadding: const EdgeInsets.fromLTRB(16, 0, 16, 16),
          // ✅ This is the key fix — tells Flutter NOT to use IntrinsicHeight
          ...
          ? ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                subtitle: Text(
                  'Description: ${t.timesheetDescription ?? ''}\nHours: ${t.hours ?? ''}',
                ),
              );
            },
          )
                ...
                () =>
                    loading.value
                        ...
                        : ListTile(
                                ...
                                onPressed: () async {
                                  ...
                                  if (confirm == true && attachmentId != null) {
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text(
                                          'Attachment ID not found. Cannot delete.',
                                        ),
                                      ),
                                    );
                                  }
                          ...
                          onTap: () async {
                            final localPath = await controller.downloadPdf(
...
Widget buildMessageSection(ticket, controller, BuildContext context) {
  ...
  WidgetsBinding.instance.addPostFrameCallback((_) {
    if (controller.message.isNotEmpty &&
        controller.messageScrollController.hasClients) {
  ...
  return list.isNotEmpty
      ? ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            title: Text(
              '${spare.productName1 ?? ''} - ${spare.Partstate1 ?? ''}',
            ),
